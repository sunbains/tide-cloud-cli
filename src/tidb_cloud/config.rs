...
mod tests {
    ...
    fn test_default_config() {
        let config = TiDBCloudConfig::default();
        assert_eq!(config.api.base_url, PRODUCTION_API_BASE_URL);
    ...
    fn test_get_api_url() {
        let config = TiDBCloudConfig::default();
        assert_eq!(config.get_api_url(), PRODUCTION_API_URL);
    }
    ...
    fn test_validation() {
        ...
        // Test valid API key
        config.api.api_key = Some("this-is-a-valid-api-key-that-is-long-enough".to_string());
        assert!(config.validate().is_ok());
    }
    ...
    fn test_environment_overrides() {
        unsafe {
            std::env::set_var("TIDB_CLOUD_API_KEY", "test-api-key-from-env");
            std::env::set_var("TIDB_CLOUD_BASE_URL", "https://test-api.example.com");
            std::env::set_var("TIDB_CLOUD_TIMEOUT_SECS", "60");
        }
        ...
        // Clean up
        unsafe {
            std::env::remove_var("TIDB_CLOUD_API_KEY");
            std::env::remove_var("TIDB_CLOUD_BASE_URL");
            std::env::remove_var("TIDB_CLOUD_TIMEOUT_SECS");
        }
    }
    ...
    fn test_from_file() {
        ...
        let config = TiDBCloudConfig::from_file("config/tidb_cloud.json").unwrap();
        assert_eq!(config.api.base_url, PRODUCTION_API_BASE_URL);
        assert_eq!(config.api.version, API_VERSION);
        assert_eq!(config.client.timeout_secs, DEFAULT_TIMEOUT_SECS);

        // Test TOML file loading
        let config = TiDBCloudConfig::from_file("config/tidb_cloud.toml").unwrap();
        assert_eq!(config.api.base_url, PRODUCTION_API_BASE_URL);
        assert_eq!(config.api.version, API_VERSION);
        assert_eq!(config.client.timeout_secs, DEFAULT_TIMEOUT_SECS);
    }
