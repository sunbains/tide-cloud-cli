...
impl TiDBCloudClient {
    ...
    /// List TiDB clusters
    pub async fn list_tidbs(&self, params: Option<&ListTidbsParams>) -> TiDBCloudResult<ListTidbsResponse> {
        let query = params.map(|p| self.build_query_string(p)).transpose()?;
        self.get("/tidbs", query.as_deref()).await
    }

    /// Create a new TiDB cluster
    pub async fn create_tidb(&self, tidb: &Tidb, validate_only: Option<bool>) -> TiDBCloudResult<Tidb> {
    ...
    pub async fn get_tidb(&self, tidb_id: &str) -> TiDBCloudResult<Tidb> {
    ...
    pub async fn update_tidb(
        ...
        if let Some(query) = query {
            ...
            let response = self.http_methods.client.patch(&url).json(update).send().await?;
    ...
    pub async fn delete_tidb(&self, tidb_id: &str, validate_only: Option<bool>) -> TiDBCloudResult<Tidb> {
        let path = PathBuilder::new().tidb_id(tidb_id).build();
        let query = if let Some(validate) = validate_only {
            QueryBuilder::new().validate_only(validate).build()
        } else {
    ...
    pub async fn reset_root_password(
        ...
        let path = PathBuilder::new().tidb_id(tidb_id).reset_root_password().build();
    ...
    pub async fn get_cloud_provider_info(&self, tidb_id: &str) -> TiDBCloudResult<CloudProviderInfo> {
        let path = PathBuilder::new().tidb_id(tidb_id).cloud_provider_info().build();
        self.get(&path, None).await
    }

    /// Estimate price for a TiDB cluster
    pub async fn estimate_price(&self, request: &EstimatePriceRequest) -> TiDBCloudResult<EstimatePriceResponse> {
    ...
    pub async fn get_backup_setting(&self, tidb_id: &str) -> TiDBCloudResult<BackupSetting> {
        let path = format!("/tidbs/{}/backupSetting", tidb_id);
        self.get(&path, None).await
    }
    ...
    pub async fn update_backup_setting(
        ...
        if let Some(query) = query {
            let url = format!("{}{}?{}", self.base_url(), path, query);
            let response = self.http_methods.client.patch(&url).json(settings).send().await?;
            self.http_methods.handle_response(response).await
        } else {
    ...
    pub async fn list_backups(&self, tidb_id: &str, params: Option<&ListBackupsParams>) -> TiDBCloudResult<ListTidbBackupsResponse> {
        let query = params.map(|p| self.build_query_string(p)).transpose()?;
        let path = format!("/tidbs/{}/backups", tidb_id);
        self.get(&path, query.as_deref()).await
    }

    /// Delete a backup
    pub async fn delete_backup(&self, tidb_id: &str, backup_id: &str) -> TiDBCloudResult<()> {
        let path = format!("/tidbs/{}/backups/{}", tidb_id, backup_id);
    ...
    pub async fn restore_tidb(&self, request: &RestoreTidbRequest) -> TiDBCloudResult<RestoreTidbResponse> {
        self.post("/tidbs:restore", Some(request)).await
    }

    /// Get restore status for a TiDB cluster
    pub async fn get_restore_status(&self, tidb_id: &str) -> TiDBCloudResult<RestoreStatusResponse> {
        let path = format!("/tidbs/{}:restoreStatus", tidb_id);
        self.get(&path, None).await
    }
}
...
impl TiDBCloudClient {
    ...
    pub async fn list_all_tidbs(&self, params: Option<&ListTidbsParams>) -> TiDBCloudResult<Vec<Tidb>> {
    ...
    pub async fn list_all_backups(&self, tidb_id: &str, params: Option<&ListBackupsParams>) -> TiDBCloudResult<Vec<Backup>> {
    ...
    pub async fn wait_for_tidb_state(
        ...
        Err(crate::tidb_cloud::error::TiDBCloudError::TimeoutError(
            format!("TiDB cluster {} did not reach state {:?} within timeout", tidb_id, target_state)
        ))
    }

    /// Wait for a TiDB cluster to be active
    pub async fn wait_for_tidb_active(&self, tidb_id: &str) -> TiDBCloudResult<Tidb> {
    ...
    pub async fn create_tidb_and_wait(&self, tidb: &Tidb) -> TiDBCloudResult<Tidb> {
    ...
    pub async fn delete_tidb_and_wait(&self, tidb_id: &str) -> TiDBCloudResult<Tidb> {
